...
void nrf_802154_receive_failed(nrf_802154_rx_error_t error, uint32_t id)
{
	...
	if (id == DRX_SLOT_RX) {
		...
		/* When CSL debug option is used we intentionally avoid notifying the higher layer about
		 * the finalization of a DRX slot, so that the radio stays in receive state for receiving
		 * "out of slot" frames.
		 * As a side effect, regular failure notifications would be reported with the incorrect ID.
		 */
		nrf5_data.event_handler(dev, IEEE802154_EVENT_SLEEP, NULL);
#endif
